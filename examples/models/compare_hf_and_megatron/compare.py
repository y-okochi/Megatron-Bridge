    ...
    try:
        ...
        try:
            ...
            if 'qwen' in model_class_name.lower():
                module_name = f'transformers.models.qwen2_vl'
            elif 'llava' in model_class_name.lower():
                module_name = f'transformers.models.llava'
            elif 'blip' in model_class_name.lower():
                module_name = f'transformers.models.blip_2'
            else:
...
def get_model_class(model_class_name: str = None, is_vl_model: bool = False):
    ...
    if model_class_name:
        ...
        # Default behavior
        if is_vl_model:
            print_rank_0("Warning: VL model detected but no model class specified. Using AutoModelForCausalLM which may not work.")
            print_rank_0("Consider using --model_class argument (e.g., --model_class Qwen2_5_VLForConditionalGeneration)")
...
def is_vision_language_model(model_path: str) -> bool:
    ...
    try:
        ...
        vl_indicators = [
            'vl', 'vision', 'multimodal', 'clip', 'blip', 'flamingo', 'llava',
            'qwen2_vl', 'qwen_vl', 'minicpm'
        ]
...
def compare_models_one_step(args) -> None:
    ...
    if torch.distributed.is_initialized():
        ...
        if hf_logits is None:
            ...
            vocab_size = getattr(tokenizer, 'vocab_size', len(tokenizer.vocab) if hasattr(tokenizer, 'vocab') else 32000)
    ...
    with torch.no_grad():
        ...
        if is_last_stage:
            ...
            if torch.distributed.is_initialized() and parallel_state.get_tensor_model_parallel_world_size() > 1:
                ...
                dist.all_gather(gathered_tensors, megatron_output, group=parallel_state.get_tensor_model_parallel_group())
            ...
            if not torch.distributed.is_initialized() or parallel_state.get_tensor_model_parallel_rank() == 0:
                ...
                print(f"Megatron next token: {megatron_next_token.item()} ('{tokenizer.decode([megatron_next_token.item()])}')")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str, 
        default=None, 
        help="Path to the Megatron model checkpoint"
    )
