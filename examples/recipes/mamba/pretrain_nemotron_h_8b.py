...
"""
Nemotron H 8B Pretraining Script with YAML and CLI Configuration Overrides.

This script provides a flexible way to pretrain Nemotron H 8B models using Megatron-Bridge with support for
both YAML configuration files and command-line overrides using Hydra-style syntax.

Examples:
    Basic usage with default configuration:
        $ python pretrain_nemotron_h_8b.py

    Using a custom YAML config file:
        $ python pretrain_nemotron_h_8b.py --config-file my_custom_config.yaml

    Using CLI overrides only:
        $ python pretrain_nemotron_h_8b.py model.tensor_model_parallel_size=4 train.train_iters=100000

    Combining YAML and CLI overrides (CLI takes precedence):
        $ python pretrain_nemotron_h_8b.py --config-file conf/my_config.yaml \
...
from megatron.bridge.recipes.mamba.nemotron_h_8b import pretrain_config
...
# Assumes this script (pretrain_nemotron_h_8b.py) is in Megatron-Bridge/examples/recipes/mamba/
# and the config is in a 'conf' subdirectory.
SCRIPT_DIR: Path = Path(__file__).parent.resolve()
DEFAULT_CONFIG_FILENAME: str = "nemotron_h_8b_pretrain_override_example.yaml"
...
def parse_cli_args() -> Tuple[argparse.Namespace, list[str]]:
    ...
    parser = argparse.ArgumentParser(
        description="Pretrain Nemotron H 8B model using Megatron-Bridge with YAML and CLI overrides",
    ...
    parser.add_argument(
        ...
        help="Path to the YAML OmegaConf override file. Default: conf/nemotron_h_8b_pretrain_override_example.yaml",
    )
...
def main() -> None:
    """
    Entry point for the Nemotron H 8B pretraining script.
    ...
    Examples of CLI usage:
        ...
        python pretrain_nemotron_h_8b.py optimizer.lr=0.0002

        # Custom config file with additional overrides
        python pretrain_nemotron_h_8b.py --config-file my_config.yaml train.train_iters=50000
    ...
    logger.info("Megatron-Bridge Nemotron H 8B Pretraining Script with YAML & CLI Overrides")
    logger.info("------------------------------------------------------------------")
