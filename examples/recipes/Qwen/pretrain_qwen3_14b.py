...
"""
Qwen3 14B Pretraining Script with YAML and CLI Configuration Overrides.

This script provides a flexible way to pretrain Qwen3 14B models using Megatron-Bridge with support for
both YAML configuration files and command-line overrides using Hydra-style syntax.

Examples:
    Basic usage with default configuration:
        $ torchrun --nproc_per_node=8 pretrain_qwen3_14b.py

    Using a custom YAML config file:
        $ torchrun --nproc_per_node=8 pretrain_qwen3_14b.py --config-file my_custom_config.yaml

    Using CLI overrides only:
        $ torchrun --nproc_per_node=8 pretrain_qwen3_14b.py model.tensor_model_parallel_size=4 train.train_iters=100000

    Combining YAML and CLI overrides (CLI takes precedence):
        $ torchrun --nproc_per_node=8 pretrain_qwen3_14b.py --config-file conf/my_config.yaml \
...
# Assumes this script (pretrain_qwen3_14b.py) is in Megatron-Bridge/examples/recipes/Qwen/
# and the config is in a 'conf' subdirectory.
SCRIPT_DIR: Path = Path(__file__).parent.resolve()
DEFAULT_CONFIG_FILENAME: str = "qwen3_14b_pretrain_override_example.yaml"
...
def parse_cli_args() -> Tuple[argparse.Namespace, list[str]]:
    ...
    parser = argparse.ArgumentParser(
        description="Pretrain Qwen3 14B model using Megatron-Bridge with YAML and CLI overrides",
    ...
    parser.add_argument(
        ...
        help="Path to the YAML OmegaConf override file. Default: conf/qwen3_14b_pretrain_override_example.yaml",
    )
...
def main() -> None:
    """
    Entry point for the Qwen3 14B pretraining script.
    ...
    Examples of CLI usage:
        ...
        torchrun --nproc_per_node=8 pretrain_qwen3_14b.py optimizer.lr=0.0002

        # Custom config file with additional overrides
        torchrun --nproc_per_node=8 pretrain_qwen3_14b.py --config-file my_config.yaml train.train_iters=50000
    ...
    logger.info("Megatron-Bridge Qwen3 14B Pretraining Script with YAML & CLI Overrides")
    logger.info("--------------------------------------------------------------------")
