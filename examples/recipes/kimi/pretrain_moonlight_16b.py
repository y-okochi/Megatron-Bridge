...
"""
Moonlight-16B Pretraining Script with YAML and CLI Configuration Overrides.

This script provides a flexible way to pretrain Moonlight-16B models using Megatron-Bridge with support for
both YAML configuration files and command-line overrides using Hydra-style syntax.

    Examples:
    Basic usage with default configuration:
        $ python pretrain_moonlight_16b.py

    Using a custom YAML config file:
        $ python pretrain_moonlight_16b.py --config-file my_custom_config.yaml

    Using CLI overrides only:
        $ python pretrain_moonlight_16b.py model.tensor_model_parallel_size=4 train.train_iters=100000

    Specifying custom data paths:
        $ python pretrain_moonlight_16b.py --data-paths /path/to/data1 /path/to/data2 /path/to/data3

    Combining YAML and CLI overrides (CLI takes precedence):
        $ python pretrain_moonlight_16b.py --config-file conf/my_config.yaml \
...
from megatron.bridge.recipes.kimi.moonlight_16b import pretrain_config
...
# Assumes this script (pretrain_moonlight_16b.py) is in Megatron-Bridge/examples/recipes/kimi/
# and the config is in a 'conf' subdirectory.
SCRIPT_DIR: Path = Path(__file__).parent.resolve()
DEFAULT_CONFIG_FILENAME: str = "moonlight_16b_pretrain_override_example.yaml"
...
def parse_cli_args() -> Tuple[argparse.Namespace, list[str]]:
    ...
    parser = argparse.ArgumentParser(
        description="Pretrain Moonlight-16B model using Megatron-Bridge with YAML and CLI overrides",
    ...
    parser.add_argument(
        ...
        help="Path to the YAML OmegaConf override file. Default: conf/moonlight_16b_pretrain_override_example.yaml",
    )
    parser.add_argument("--debug", action="store_true", help="Enable debug logging")
    parser.add_argument("--optimizer-type", type=str, default="muon", help="Optimizer type: [adam, muon]")
    parser.add_argument(
        "--data-paths", 
        type=str,
        nargs='+',
...
def main() -> None:
    """
    Entry point for the Moonlight-16B pretraining script.
    ...
    Examples of CLI usage:
        ...
        python pretrain_moonlight_16b.py optimizer.lr=0.0003

        # Custom config file with additional overrides
        python pretrain_moonlight_16b.py --config-file my_config.yaml train.train_iters=500000
        ...
        python pretrain_moonlight_16b.py \
            ...
            train.global_batch_size=2048
    """
    args, cli_overrides = parse_cli_args()

    logger.info("Megatron-Bridge Moonlight-16B Pretraining Script with YAML & CLI Overrides")
    logger.info("------------------------------------------------------------------")

    # Load base configuration from the recipe as a Python dataclass
    cfg: ConfigContainer = pretrain_config(optimizer_type=args.optimizer_type, data_paths=args.data_paths)
