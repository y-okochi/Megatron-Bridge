...
"""
NeMo Run Launcher for Moonlight-16B Pretraining.

This script launches the pretrain_moonlight_16b.py script using NeMo Run with TorchRun,
while forwarding any additional command line arguments to the target script.

Examples:
    Basic usage with default config:
        $ python pretrain_moonlight_16b_nemo_run_script.py --nproc-per-node=8

    Using a custom config file:
        $ python pretrain_moonlight_16b_nemo_run_script.py --nproc-per-node=8 --config-file=my_config.yaml
    ...
    Dry run to see what would be executed:
        $ python pretrain_moonlight_16b_nemo_run_script.py --nproc-per-node=8 --dryrun \
            model.pipeline_dtype=torch.bfloat16

Argument Forwarding:
    Any arguments not recognized by this launcher script will be forwarded
    to the target pretrain_moonlight_16b.py script as Hydra-style overrides.
"""
...
# Assumes this script (pretrain_moonlight_16b_nemo_run_script.py) is in Megatron-Bridge/examples/recipes/kimi/
# and pretrain_moonlight_16b.py is in the same directory,
# and the config is in a 'conf' subdirectory.
SCRIPT_DIR: Path = Path(__file__).parent.resolve()
PRETRAIN_SCRIPT_FILENAME: str = "pretrain_moonlight_16b.py"
...
def parse_cli_args() -> Tuple[argparse.Namespace, list[str]]:
    ...
    parser = argparse.ArgumentParser(
        description="Launcher for Moonlight-16B pretraining using nemo_run and TorchRun. "
        "Additional arguments will be forwarded to pretrain_moonlight_16b.py",
    ...
    parser.add_argument(
        ...
        help="Path to the YAML override config file for the pretrain_moonlight_16b.py script.",
    )
    parser.add_argument(
        "--dryrun",
        action="store_true",
...
def main() -> None:
    ...
    logger.info("Nemo Run Launcher for Moonlight-16B Pretraining")
    logger.info("=============================================")
