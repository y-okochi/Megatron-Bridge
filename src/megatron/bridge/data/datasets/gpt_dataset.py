...
# =============================================================================
# Mock Qwen2.5-VL Dataset (text + image via HF Processor)
# =============================================================================
...
@dataclass
class MockQwen25VLConfig:
    """Configuration for the mock Qwen2.5-VL dataset.

    This dataset generates synthetic images and simple prompts, then uses a
    HuggingFace AutoProcessor (for Qwen2.5-VL) to produce model-ready inputs:
      - input_ids (tokens)
      - pixel_values (vision tensor)
      - image_grid_thw (vision grid info)

    The dataset returns a dictionary compatible with Megatron forward steps,
    including language training fields and the vision fields.
    """

    processor: Any
    """HF AutoProcessor instance for Qwen2.5-VL (required)."""

    prompt: str = "Describe this image."
    """Simple prompt to accompany the image."""

    sequence_length: int = 256
    """Max sequence length for padding/truncation of input_ids."""

    random_seed: int = 0
    """Seed for reproducible synthetic data generation."""

    image_size: Tuple[int, int] = (256, 256)
    """Size (width, height) of the generated synthetic images."""

    pad_to_max_length: bool = True
    """If True, pad input_ids to sequence_length via processor."""

    create_attention_mask: bool = True
    """If True, dataset provides an attention mask (1D valid-token mask)."""

    reset_position_ids: bool = False
    reset_attention_mask: bool = False
    eod_mask_loss: bool = False
    """Flags mirroring GPT dataset behavior for position/loss masking."""
...
class MockQwen25VLDataset(torch.utils.data.Dataset):
    ...
    Each sample contains:
      ...
      - pixel_values: torch.FloatTensor [C, H, W]
      - image_grid_thw: torch.LongTensor [1, 3]
    """
    ...
    def _build_inputs(self) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        pixel_values: torch.Tensor = inputs.pixel_values[0]  # [C, H, W]
        image_grid_thw: torch.Tensor = inputs.image_grid_thw  # [num_images, 3]
