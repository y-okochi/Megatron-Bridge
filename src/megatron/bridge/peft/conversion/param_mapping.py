...
class AdapterMappingFactory:
    ...
    def _column_parallel_scatter(self, tensor: torch.Tensor) -> torch.Tensor:
        ...
        return self.scatter_to_tp_ranks(
            list(chunks),
            output_shape=torch.Size(out_shape),
            dtype=tensor.dtype,
...
class AdapterQKVMapping(MegatronParamMapping[Dict[str, torch.Tensor]]):
    ...
    def from_base_mapping(
        ...
        """
        adapter_hf_params = {
            k: v.replace(".weight", hf_suffix)
            for k, v in base_mapping.hf_param.items()
        }
        return cls(
            adapter_megatron_param,
            q=adapter_hf_params["q"],
            k=adapter_hf_params["k"],
            v=adapter_hf_params["v"]
        )
...
class AdapterGatedMLPMapping(MegatronParamMapping[Dict[str, torch.Tensor]]):
    ...
    def from_base_mapping(
        ...
        return cls(
            adapter_megatron_param,
            gate=adapter_hf_params["gate"],
            up=adapter_hf_params["up"]
        )
