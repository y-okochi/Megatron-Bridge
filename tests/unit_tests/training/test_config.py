...
>>>>>>> Stashed changes
    ...
    def test_cuda_graph_mutual_exclusivity_error(self):
        ...
        with pytest.raises(AssertionError, match="enable_cuda_graph and external_cuda_graph cannot be enabled at the same time"):
            container._sync_and_validate_external_cuda_graph()
...
>>>>>>> Stashed changes
        ...
        with pytest.raises(AssertionError, match="expandable_segments:True may not be safe when using CUDA Graphs"):
            container._sync_and_validate_external_cuda_graph()
